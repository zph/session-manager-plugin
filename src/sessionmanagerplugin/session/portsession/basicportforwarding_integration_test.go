...
func TestSetSessionHandlers(t *testing.T) {
	...
	// Use Maybe() since SendMessage may or may not be called depending on timing
	mockWebSocketChannel.On("SendMessage", mockLog, mock.Anything, mock.Anything).
		Return(countTimes()).Maybe()
	...
	// Don't assert counter==1 since SendMessage may not be called due to race conditions
	// The main purpose of this test is to verify signal handling
